...
const MoviePanel = ({movie, buttonSettings}) => {
            ...
            <div className='col-start-2 col-end-6 ...'>
                <div className='py-8 ml-6'>
                    <article>
                        ...
                        <ul className='text-sm pb-2'><li className='inline pr-1'>{releaseDate.getFullYear()}</li><li className='inline pr-1'>
                            <Link to='/classifications'>{movie.Rated}</Link></li><li className='inline pr-1'>{movie.Runtime}</li></ul>
                        <h2 className='pb-3'>{movie.Actors}</h2>
                        <p className='text-sm pb-5'>{movie.Plot}</p>
                    </article>
                    <Link to={buttonSettings.link}>
                        <button className='custom-button' onClick={() => setChosenFilmId(movie._id)}>
